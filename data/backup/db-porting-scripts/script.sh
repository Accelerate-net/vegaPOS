...
echo
echo [25/25]
stub_data_received=$(curl https://raw.githubusercontent.com/Accelerate-net/vegaPOS/master/data/backup/settings-db-stub/ZAITOON_KOT_RELAYING.json)
curl -X PUT http://$database_user_name:$database_user_password@$database_ip:$database_port/zaitoon_settings/ZAITOON_KOT_RELAYING -d "$stub_data_received"
...
echo
echo [12/15]
stub_data_received=$(curl https://raw.githubusercontent.com/Accelerate-net/vegaPOS/master/data/backup/couchdb-apis/couchdb_invoice_order_summary.js)
curl -X PUT http://$database_user_name:$database_user_password@$database_ip:$database_port/zaitoon_invoices/_design/order-summary -d "$stub_data_received"
#ONLINE ORDERS
echo
echo [13/15]

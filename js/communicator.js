...
function sendToPrinter(orderObject, type, optionalTargetPrinter, optionalMetaData){
...
/*
   PRINTING ORIGINAL DUPLICATE BILL
*/

if(type == 'DUPLICATE_ORIGINAL_BILL'){
...
if(orderObject.orderDetails.modeType == 'DELIVERY'){
   ...
   var isAddressAddedOnDuplicate = false;
   var addressData = {};

   if(!optionalMetaData || optionalMetaData == {} || optionalMetaData == undefined){

   }
   else{
      addressData = optionalMetaData;
      isAddressAddedOnDuplicate = true;
   }

   if(isAddressAddedOnDuplicate){
       userInfo = '<td style="vertical-align: top; position: relative">'+
                     '<p>'+'<label class="subLabel">Billed To</label>'+
                        '<tag class="billingAddress">'+
                        (addressData.name != '' ? addressData.name+'<br>' : '')+
                        (addressData.flatNo != '' ? addressData.flatNo+'<br>' : '')+
                        (addressData.flatName != '' ? addressData.flatName+'<br>' : '')+
                        (addressData.landmark != '' ? addressData.landmark+'<br>' : '')+
                        (addressData.area != '' ? addressData.area+'<br>' : '')+
                        (addressData.remarks != '' ? addressData.remarks+'<br>' : '')+
                        (addressData.contact != '' ? '<tag class="mobileNumber">Mob. <b>'+addressData.contact+'</b>' : '')+ '</tag>'+
                     '</p>'+
                  '</td>';  
...
var html_template = view_header_content +
   ...
   function postContentToTemplate(html_template){
        ...
        if(type == 'DUPLICATE_ORIGINAL_BILL'){ //TWEAK
            type = 'DUPLICATE_BILL';
        }
